...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("after resnet_encoder " , x.shape)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, is_training=True):
        # for i in range(len(x)):
            # print("layout encoder shape " + str(i) + "----", x[i].shape)
        """
        ...
        for i in range(4, -1, -1):
            ...
            # print("after upconv " + str(i)+" 0", x.shape)
            x = self.convs[("norm", i, 0)](x)
            # print("after norm " + str(i)+" 0", x.shape)
            x = self.convs[("relu", i, 0)](x)
            # print("after relu "+str(i)+" 0", x.shape)
            x = upsample(x)
            # print("after upsample " + str(i), x.shape)
            x = self.convs[("upconv", i, 1)](x)
            # print("after upconv " + str(i) + " 1", x.shape)w
            x = self.convs[("norm", i, 1)](x)
            # print("after norm " + str(i) + " 1", x.shape)

        if is_training:
            x = self.convs["topview"](x)
            # print("after topview " + str(i), x.shape)
        else:
