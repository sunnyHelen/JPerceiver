...
def predict(prev_img, cv2_img, model):
    ...
    layout_object = outputs["topviewB"].squeeze().cpu().numpy()
    # if prev_img is not None:
    ...
    return depth, disp_resized.squeeze().cpu().numpy(), layout, layout_object, T_
...
def evaluate(cfg_path, model_path, img_path, output_path):
    ...
    pos_pred = [orig.copy()]
    record_dir = img_path
    sequence = img_path.split("argoverse-tracking/")[1].split("/stereo_front_left")[0]
    vid_name = 'sequence_{}_gt.avi'.replace('/', '_').format(sequence)
    vid_name = 'sequence_argo_val_.avi'
    out = cv2.VideoWriter(vid_name, cv2.VideoWriter_fourcc(*'XVID'), 10.0, (512 + 512 + 512, 512 * 2))
    T_pred = []
    T_gt = np.eye(4)
    T_pr = np.eye(4)
    if os.path.isfile(img_path):
        ...
        paths = glob.glob(os.path.join(img_path, '*.{}'.format("jpg")))
    ...
    with torch.no_grad():
        ...
        for idx, image_path in enumerate(sorted(os.listdir(img_path))):
            ...
            input_image_resized_write = cv2_img.resize((512, 512), pil.LANCZOS)
            # if prev_img is None:
            depth, disp_resized, true_top_view, true_top_view_object, T_ = predict(prev_img, cv2_img, model)

            vmax = np.percentile(disp_resized, 95)
            output_name = os.path.splitext(os.path.basename(image_path))[0]
            output_path = os.path.join(output_directory, "{}_disp.jpeg".format(output_name))
            # print(output_path)
            ...
            layout_gt_name = image_path.replace("stereo_front_left/", "both_bev_gt_new/").replace("jpg", "png")
            layout_gt = get_static(layout_gt_name).resize((512, 512), pil.LANCZOS)
            if prev_img is not None:
                T_pr = T_pr.dot(T_)
                T_pred += [T_]
            ...
            disp_np = cv2.resize(disp_resized, (512, 512))
            vmax = np.percentile(disp_np, 95)
            output = np.zeros((512 * 2, 512 + 512 + 512, 3), dtype=np.uint8)
            buf = io.BytesIO()
            ...
            layout_object = cv2.resize(layout_object, (512, 512))
            buf = io.BytesIO()
            cv2.rectangle(layout_object, (241, 472), (271, 512), (0, 255, 255), thickness=-1)
            ...
            buf.close()
            # print(output.shape)
            output[:512, :512] = np.array(input_image_resized_write)
            output[512:, :512] = disp_
            output[:512, 512:512 + 512] = layout_
            output[512:, 512:512 + 512] = vo
            output[:512, -512:] = layout_gt_
            out.write(output[:, :, ::-1])
...
if __name__ == "__main__":
    cfg_path = '/CV/zhaohaimei3/DepthLayout_kitti_odometry_loss_transformerright_object_argoverse_A100-1024-multibranch/config/cfg_kitti_baseline_argo_both_boundary_ce_iou_1024_20_B1.py'# path to cfg file
    ...
    model_path = '/CV/zhaohaimei3/DepthLayout_kitti_odometry_loss_transformerright_object_argoverse_A100-1024-multibranch/log/argo_both_boundary_ce_iou/epoch_62.pth'# path to model weight
    img_path = '/CV/datasets/kitti_data/argoverse/argoverse-tracking/train3/00c561b9-2057-358d-82c6-5b06d76cebcf/stereo_front_left/'
    output_path = '/CV/zhaohaimei3/DepthLayout_kitti_odometry_loss_transformerright_object_argoverse_A100-1024-multibranch/pred_depth_layout/' # dir for saving depth maps
